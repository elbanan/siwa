...
export default function CaptioningAnnotatePage({
  params,
}: {
  params: { id: string };
}) {
  ...
  const loadSummary = useCallback(async () => {
    ...
    const res = await api.get(
      `/datasets/${datasetId}/annotations/captioning/summary`
    );
    setSummary(res.data);
  }, [datasetId]);
  ...
  useEffect(() => {
    ...
    api
      .get(`/datasets/${datasetId}/annotations/captioning`, {
  ...
  const save = async (nextIndex?: number) => {
    ...
    const status = trimmed ? "labeled" : "unlabeled";
    await api.post(`/datasets/${datasetId}/annotations/captioning`, {
      path: currentPath,
      caption: trimmed,
      status,
    });
  ...
  return (
    <div className="space-y-3">
      <div className="bg-white border rounded-2xl p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <div className="font-semibold text-lg text-slate-900">{ds.name}</div>
      ...
      <div className="grid lg:grid-cols-3 gap-3">
        ...
        {/* Caption editor */}
        <div className="lg:col-span-1 bg-white border rounded-2xl p-3 shadow-sm space-y-3">
          <div>
            <div className="font-semibold text-sm">Describe this image</div>
            <div className="text-xs text-gray-500">
              Write a concise caption that captures the scene.
            </div>
          </div>
          <textarea
            ref={captionRef}
            className="w-full border rounded-xl px-3 py-2 text-sm min-h-[180px] focus:outline-none focus:ring-2 focus:ring-black/50 resize-none overflow-hidden"
            placeholder="E.g. “Two doctors reviewing a CT scan in a bright lab.”"
            value={caption}
            onChange={(e) => setCaption(e.target.value)}
          />
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span>
              Status: <span className="font-medium text-gray-700">{itemStatus}</span>
            </span>
            <span>{caption.trim().length} chars</span>
          </div>
